...
#   the new memory is word-aligned, and is filled with garbage
...
.mallocWordAligned:
    ...
    li   $a0,0
    jal  .sbrk
    # if (addr % 4 )==0 /* it is aligned */ goto .fillgarbage
    and  $t0,$v0,3
    beq  $t0,$zero,.mallocGetMemory
    # ok, we have a problem - memory is not aligned. move the break value
    move $a0,$t0
    jal  .sbrk
    lw  $a0,20($sp)
    jal  .sbrk
    beq  $v0,$zero,.mallocGetMemory
    and  $t0,$v0,3
    beq  $t0,$zero,.mallocGetMemory
    # cant align memory!!!
    .data
.CantAlignMsg: .asciiz "!! INTERNAL !! Cant align memory\n"
    .align 2
    .text
    la  $a0,.CantAlignMsg
    jal PrintString
...
.mallocGetMemory:
    ...
    jal .sbrk
.mallocFillGarbage:
    # okay, now fill  the new memory with garbage. It is an integral number
    # of words already.
    # char *temp = returnptr;
    move  $t0,$v0
    # char *endptr = returnptr + nbytes (word-aligned)
    lw    $a0,20($sp)
    add   $t1,$t0,$a0   #endptr
    li    $t2,0x86DEAD86
    # for (;temp < endptr; temp++) *temp = 0x86DEAD86
.mallocFillNext:
    bge   $t0,$t1,.mallocExit
    sw    $t2,0($t0)
    addiu $t0,$t0,4
    b     .mallocFillNext
.mallocExit:
...
#
# void free(void *ptr)
#   frees the memory associated with ptr. Well, not really...
    .globl free
free:
    jr  $ra
#
